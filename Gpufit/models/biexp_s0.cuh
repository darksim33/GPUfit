#ifndef GPUFIT_BIEXPRED_S0_CUH_INCLUDED
#define GPUFIT_BIEXPRED_S0_CUH_INCLUDED
...
*
* The reduced biexponential function is: S = (a*exp(-b*x) + (1-a)*exp(-c*x))*d
* The derivatives are:
...
__device__ void calculate_biexp_s0(
    ...
    std::size_t const user_info_size)
{
    ...
    value[point_index] = (p[0] * exp(-p[1]*x) + (1 - p[0])*exp(-p[2]*x))*p[3];
    ...
    current_derivatives[0 * n_points] = (exp(-p[1]*x) - exp(-p[2]*x)) * p[3];
    current_derivatives[1 * n_points] = (p[0] * (-x) * exp(-p[1] * x)) * p[3];
    current_derivatives[2 * n_points] = (1-p[0]) * (-x) * exp(-p[2]*x) * p[3];
    current_derivatives[3 * n_points] = (p[0] * exp(-p[1]*x) + (1 - p[0])*exp(-p[2]*x));    
}
