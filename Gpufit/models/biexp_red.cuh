#ifndef GPUFIT_BIEXP_RED_CUH_INCLUDED
#define GPUFIT_BIEXP_RED_CUH_INCLUDED
    ...
    std::size_t const user_info_size)
{
    ...
    value[point_index] = p[0] * exp(-p[1] * x) + (1.0 - p[0]) * exp(-p[2] * x);
    ...
    current_derivatives[0 * n_points] = exp(-p[1] * x) - exp(-p[2] * x);
