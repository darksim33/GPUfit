...
*
* The reduced biexponential function is: S = ((a*exp(-b*x) + (1-a)*exp(-c*x))*d)*(1-exp(-TR/e)))
* The derivatives are:
* dy/da = (exp(-b*x) - exp(-c*x))*d*(1-exp(-TR/e)))
* dy/db = -a*d*x*exp(-b*x)*(1-exp(-TR/e)))
* dy/dc = (a-1)*d*x*exp(-c*x)*(1-exp(-TR/e)))
* dy/dd = a*exp(-b*x) + (1-a)*exp(-c*x)*(1-exp(-TR/e)))
* dy/de = ((a*exp(-b*x) + (1-a)*exp(-c*x))*d)*(TR/e²))*exp(-TR/e)
*
...
__device__ void calculate_biexp_s0_t1(
    ...
    std::size_t const user_info_size)
{
    ...
    p[0]: a (f2)   p[1]: b  (D1)   p[2]: c   (D2)   p[3]: d   (S0)  */
    value[point_index] = (p[0] * exp(-p[1]*x) + (1 - p[0])*exp(-p[2]*x))*p[3] * (1 - exp(-TR/p[4]));

    /* derivatives
    dy/da = (exp(-b*x) - exp(-c*x))*d
    dy/db = -a*d*x*exp(-b*x)
    dy/dc = (a-1)*d*x*exp(-c*x)
    dy/dd = a*exp(-b*x) + (1-a)*exp(-c*x)
    p[0]: a (f1)   p[1]: b  (D1)   p[2]: c   (D2)   p[3]: d   (S0)  */

    REAL* current_derivatives = derivative + point_index;
    current_derivatives[0 * n_points] = (exp(-p[1]*x) - exp(-p[2]*x)) * p[3]* (1 - exp(-TR/p[4]));
    current_derivatives[1 * n_points] = (p[0] * (-x) * exp(-p[1] * x)) * p[3]* (1 - exp(-TR/p[4]));
    current_derivatives[2 * n_points] = (1-p[0]) * (-x) * exp(-p[2]*x) * p[3]* (1 - exp(-TR/p[4]));
    current_derivatives[3 * n_points] = (p[0] * exp(-p[1]*x) + (1 - p[0])*exp(-p[2]*x))* (1 - exp(-TR/p[4])); 
    // IVIM * (-TR/e²) * exp(-TR/e)   
    current_derivatives[4 * n_points] = 
    (p[0] * exp(-p[1]*x) + 
    (1 - p[0])*exp(-p[2]*x))*
    p[3]*
    ( -TR / (p[4] * p[4]))*exp(-TR/p[4]);
}
