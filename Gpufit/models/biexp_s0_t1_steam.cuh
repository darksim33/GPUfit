...
#define GPUFIT_BIEXPRED_S0_T1_STEAM_CUH_INCLUDED
...
*
* The reduced biexponential function is: S = ((a*exp(-b*x) + (1-a)*exp(-c*x))*d)*(1-exp(-TR/e)))*(exp(-TM/e)))
* The derivatives are:
* dy/da = (exp(-b*x) - exp(-c*x))*d*(1-exp(-TR/e)))*(exp(-TM/e)))
* dy/db = -a*d*x*exp(-b*x)*(1-exp(-TR/e)))*(exp(-TM/e)))
* dy/dc = (a-1)*d*x*exp(-c*x)*(1-exp(-TR/e)))*(exp(-TM/e)))
* dy/dd = a*exp(-b*x) + (1-a)*exp(-c*x)*(1-exp(-TR/e)))*(exp(-TM/e)))
* dy/de = ((a*exp(-b*x) + (1-a)*exp(-c*x))*d)*(TM/e^2 - (TM + TR)/e^2 * exp(-TR/e)) * exp(-TM/e)
*
...
*             p[0]: a   p[1]: b     p[2]: c  p[3]: d  p[4]: e  p[5]: e 
*
...
__device__ void calculate_biexp_s0_t1_steam(
    ...
    std::size_t const user_info_size)
{
    ...
    p[0]: a (f2)   p[1]: b  (D1)   p[2]: c   (D2)   p[3]: d   (S0)  p[4]: e   (T1)  */
    value[point_index] = (p[0] * exp(-p[1]*x) + (1 - p[0])*exp(-p[2]*x))*p[3] * (1 - exp(-TR/p[4])) * (exp(-TM/p[4]));
    ...
    dy/dc = (a-1)*d*x*exp(-c*x)*(1-exp(-TR/e)))*(exp(-TM/e)))
    dy/dd = a*exp(-b*x) + (1-a)*exp(-c*x)*(1-exp(-TR/e)))*(exp(-TM/e)))
    dy/de = ((a*exp(-b*x) + (1-a)*exp(-c*x))*d)*(TM/T1 - (TM + TR)/T1 * exp(-TR/T1)) * exp(-TM/T1)
    p[0]: a (f1)   p[1]: b  (D1)   p[2]: c   (D2)   p[3]: d   (S0)  p[4]: e   (T1)  
    */

    REAL* current_derivatives = derivative + point_index;
    current_derivatives[0 * n_points] = (exp(-p[1]*x) - exp(-p[2]*x)) * p[3]* (1 - exp(-TR/p[4])) * (exp(-TM/p[4]));
    current_derivatives[1 * n_points] = (p[0] * (-x) * exp(-p[1] * x)) * p[3]* (1 - exp(-TR/p[4])) * (exp(-TM/p[4]));
    current_derivatives[2 * n_points] = (1-p[0]) * (-x) * exp(-p[2]*x) * p[3]* (1 - exp(-TR/p[4])) * (exp(-TM/p[4]));
    current_derivatives[3 * n_points] = (p[0] * exp(-p[1]*x) + (1 - p[0])*exp(-p[2]*x)) * (1 - exp(-TR/p[4])) * (exp(-TM/p[4])); 
