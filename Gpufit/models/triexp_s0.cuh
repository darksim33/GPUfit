#ifndef GPUFIT_TRIEXP_S0_CUH_INCLUDED
#define GPUFIT_TRIEXP_S0_CUH_INCLUDED
...
*
* The triexponential function is: S = (a*exp(-b*x)+c*exp(-d*x)+(1-a-c)*exp(-e*x))*f
* The derivatives are:
* dy/da = (exp(-bx) - exp(-ex))*f
* dy/db = (a*(-x)*exp(-bx))*f
* dy/dc = (exp(-dx) - exp(-ex))*f  //a x e^(-c x) + b x e^(-c x) - x e^(-c x)
* dy/dd = c*(-x)*exp(-d*x)*f
* dy/de = (1-a-c)*(-x)*exp(-e*x)*f
* dy/df = a*exp(-b*x)+c*exp(-d*x)+(1-a-c)*exp(-e*x)
*
...
__device__ void calculate_triexp_s0(
    ...
    std::size_t const user_info_size)
{
    ...
    */
    ...
    current_derivatives[0 * n_points] = (exp(-p[1] * x) - exp(-p[4] * x)) * p[5];
    current_derivatives[1 * n_points] = (p[0] * (-x * exp(-p[1] * x))) * p[5];
    current_derivatives[2 * n_points] = (exp(-p[3] * x) - exp(-p[4] * x)) * p[5];
    current_derivatives[3 * n_points] = (p[2] * (-x * exp(-p[3] * x))) * p[5];
    current_derivatives[4 * n_points] = ((1-p[0]-p[2]) * (-x * exp(-p[4] * x))) * p[5];
    current_derivatives[5 * n_points] = p[0] * exp(-p[1] * x) + p[2] * exp(-p[3] * x) + (1 - p[0] - p[2]) * exp(-p[4] * x);
}
#endif
