#ifndef GPUFIT_MONOEXP_T1_CUH_INCLUDED
#define GPUFIT_MONOEXP_T1_CUH_INCLUDED
 ...
 *
 * The monoexponential function is: y = a*exp(-b*x) * (1 - exp(-TR/c))
 * The derivatives are:
 * dy/da = exp(-b*x) * (1 - exp(-TR/c))
 * dy/db = a*(-x)*exp(-b*x) * (1 - exp(-TR/c))
 * dy/dc = a*exp(-b*x) * (1 - exp(-TR/c)) * (TR/c²)
 *
...
__device__ void calculate_monoexp_t1(
    ...
    std::size_t const user_info_size)
{
    ...
    p[0]: a   p[1]: b   p[2]: c     */
    value[point_index] = p[0] * exp(-p[1] * x) * (1 - exp(-TR / p[2]));
    ...
    */
    ...
    current_derivatives[0 * n_points] = exp(-p[1] * x) * (1 - exp(-TR / p[2]));
    current_derivatives[1 * n_points] = p[0] * (-x) * exp(-p[1] * x) * (1 - exp(-TR / p[2]));
    current_derivatives[2 * n_points] = p[0] * exp(-p[1] * x) * (-TR / (p[2] * p[2])) * exp(-TR / p[2]);
}

#endif // GPUFIT_MONOEXP_T1_CUH_INCLUDED