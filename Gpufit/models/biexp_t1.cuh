 ...
 *
 * The reduced biexponential function is: y = (a*exp(-b*x)+c*exp(-d*x))*(1-exp(-TR/e))
 * The derivatives are:
 * dy/da = exp(-b*x)*(1-exp(-TR/e))
 * dy/db = a*(-x)*exp(-b*x)*(1-exp(-TR/e))
 * dy/dc = exp(-d*x)*(1-exp(-TR/e))
 * dy/dd = c*(-x)*exp(-d*x)*(1-exp(-TR/e))
 * dy/de = (a*exp(-b*x)+c*exp(-d*x))*(TR/e²)*exp(-TR/e)
 *
    ...
    std::size_t const user_info_size)
{
    ...
    dy/dc = exp(-d*x)*(1-exp(-TR/e))
    dy/dd = c*(-x)*exp(-d*x)*(1-exp(-TR/e))
    dy/de = (a*exp(-b*x)+c*exp(-d*x)) * (-TR / (e * e)) * exp(-TR / e)
    */
    REAL *current_derivatives = derivative + point_index;
    current_derivatives[0 * n_points] = exp(-p[1] * x) * (1 - exp(-TR / p[4]));
    current_derivatives[1 * n_points] = p[0] * (-x) * exp(-p[1] * x) * (1 - exp(-TR / p[4]));
    current_derivatives[2 * n_points] = exp(-p[3] * x) * (1 - exp(-TR / p[4]));
    current_derivatives[3 * n_points] = p[2] * (-x) * exp(-p[3] * x) * (1 - exp(-TR / p[4]));
    // IVIM * (-TR/e²) * exp(-TR/e)
    current_derivatives[4 * n_points] = (p[0] * exp(-p[1] * x) + p[2] * exp(-p[3] * x)) * (-TR / (p[4] * p[4])) * exp(-TR / p[4]);
}
