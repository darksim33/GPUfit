#ifndef GPUFIT_TRIEXP_S0_T1_STEAM_CUH_INCLUDED
#define GPUFIT_TRIEXP_S0_T1_STEAM_CUH_INCLUDED
 ...
 * The derivatives are:
 * dy/da = (exp(-bx) - exp(-ex))*f*(1 - exp(-TR/g))* (exp(-TM/g))
 * dy/db = (a*(-x)*exp(-bx))*f*(1 - exp(-TR/g))* (exp(-TM/g))
 * dy/dc = (exp(-dx) - exp(-ex))*f*(1 - exp(-TR/g))* (exp(-TM/g))
 * dy/dd = c*(-x)*exp(-d*x)*f*(1 - exp(-TR/g))* (exp(-TM/g))
 * dy/de = (1-a-c)*(-x)*exp(-e*x)*f*(1 - exp(-TR/g))* (exp(-TM/g))
 * dy/df = a*exp(-b*x)+c*exp(-d*x)+(1-a-c)*exp(-e*x)*(1 - exp(-TR/g))* (exp(-TM/g))
 * dy/dg = (a*exp(-b*x)+c*exp(-d*x)+(1-a-c)*exp(-e*x))*f**(TM/g^2 - (TM + TR)/g^2 * exp(-TR/g)) * exp(-TM/g)
 *
...
__device__ void calculate_triexp_s0_t1_steam(
    ...
    std::size_t const user_info_size)
{
    ...
    */
    ...
    current_derivatives[2 * n_points] =
        (exp(-p[3] * x) -
         exp(-p[4] * x)) *
        p[5] *
        (1 - exp(-TR / p[6])) * (exp(-TM / p[6]));

    current_derivatives[3 * n_points] =
        (p[2] * (-x * exp(-p[3] * x))) *
        p[5] *
        (1 - exp(-TR / p[6])) * (exp(-TM / p[6]));

    current_derivatives[4 * n_points] =
        ((1 - p[0] - p[2]) * (-x * exp(-p[4] * x))) *
        p[5] *
        (1 - exp(-TR / p[6])) * (exp(-TM / p[6]));

    current_derivatives[5 * n_points] =
        (p[0] * exp(-p[1] * x) +
         p[2] * exp(-p[3] * x) +
         (1 - p[0] - p[2]) * exp(-p[4] * x)) *
        (1 - exp(-TR / p[6])) * (exp(-TM / p[6]));
    ...
    current_derivatives[6 * n_points] =
        (p[0] * exp(-p[1] * x) +
         ...
         (1 - p[0] - p[2]) * exp(-p[4] * x)) *
        p[5] *
        ((TM / (p[6] * p[6])) - ((TM + TR) / (p[6] * p[6])) * exp(-TR / p[6])) * (exp(-TM / p[6]));
}
#endif
